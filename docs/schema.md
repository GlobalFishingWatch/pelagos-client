...
Tiles are loaded from under any of the provided fallback urls by adding the
tile bounding box:
...
[tileParamsForRegion()](https://github.com/SkyTruth/pelagos-client/blob/master/js/app/Data/BaseTiledFormat.js#L188).
...
Some layers are reportable, which means that a report can be generated based on
that layer. Reports are generated asynchronously, and the generation is
triggered by posting to a url like this one:

`http://myproject.appspot.com/tile/mytileset/sub/layer/report`

The specifics of what needs to be posted in the request body depends on the
layer that is reported.
...
```
{
  ...
  /* If present, indicates that a report is available for this animation */
  "reportable": true,

  /* Base url to report on */
  "reportBaseUrl": "http://myproject.appspot.com/tiles/mytileset",
